...
export function StudentHistoryModal({
  ...
  useEffect(() => {
    getStudentAttendanceHistory(userId).then((result) => {
      if (result.success) {
        setRecords((result.data || []) as unknown as AttendanceRecord[])
      } else {
              ...
              {records.map((record) => (
                <div key={record.id} className="flex items-center gap-3 p-3 bg-gray-50 rounded-xl">
                  ...
                  <div className="text-xs text-gray-500 font-medium whitespace-nowrap">
                    {new Date(record.scanned_at).toLocaleDateString('es-CO', {
                      day: '2-digit',
                      month: '2-digit',
                      year: 'numeric',
                    })}{' '}
                    {new Date(record.scanned_at).toLocaleTimeString('es-CO', {
                      hour: '2-digit',
                      minute: '2-digit',
                    })}
                  </div>
...
export function ProfessorCareersModal({
  professorId,
  professorName,
  careers,
  initialCareerIds,
  ...
  const handleToggle = async (career: Career) => {
    const isChecked = checked.has(career.id)
    setLoadingId(career.id)
    const result = await setProfessorCareer(professorId, career.id, !isChecked)
    if (result.success) {
      setChecked((prev) => {
        const next = new Set(prev)
        if (isChecked) next.delete(career.id)
      ...
      showToast(
        `${professorName} ${isChecked ? 'removido de' : 'agregado a'} ${career.name}.`,
        'success'
      )
    } else {
      showToast(result.error || 'No se pudo actualizar la carrera.', 'error')
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-7 gap-5 items-end">
        ...
        <div>
          ...
          <input
            ...
            name="email"
            type="email"
            placeholder="correo@urepublicana.edu.co"
            className={inputClass}
          />
        </div>
        <div>
          ...
          <div className="space-y-4 mb-6">
            ...
            <div>
              ...
              <input
                value={code}
                onChange={(e) =>
                  setCode(e.target.value.replace(/\D/g, '').slice(0, isStudent ? 12 : 20))
                }
                type="text"
                inputMode="numeric"
